...
class Member_model extends CI_Model
{
    ...
    private $_column_search = array('t_user.id', 't_user.name', 't_user.username', 't_user.email');
    ...
    private function _datatable_query()
    {
        ...
        $this->db->select('
					     ...
					     t_user.status_data       AS  status_data,
					    ');
        ...
        if (!empty($this->input->post('order'))) {
            ...
            $this->db->order_by('t_user.status_data', 'ASC');
    ...
    {
        return $this->db->get_where('t_wallet', array('user_id' => $id))->row_array()['amount'];
    }
    ...
    public function get_user($id)
    {
        ...
        $query = $this->db
            ->select('
				     ...
				     t_user.id_type      AS  user_idtype,
				     t_user.id_number    AS  user_idnumber,
				     t_user.id_photo     AS  user_idphoto,
				     t_user.photo        AS  user_photo,
				     t_user.active       AS  user_active,
				     t_user.status_data        AS  user_status_data,
				    ');
    ...
    public function get_idphoto($id)
    {
        if ($this->cek_id($id) > 0) {
            $query = $this->db->where('id', $id);
            $query = $this->db->get('t_user');
            $result = $query->row_array();
            $photo = $result['id_photo'];
            return $photo;
        }
        else {
            return NULL;
