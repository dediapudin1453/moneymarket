...
class Menuwebsite extends Admin_controller {

	public $mod = 'menuwebsite';
	public $pk;
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/menuwebsite_model','mod_model');
	}
	...
	public function index()
	{
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				{
					$parent = $this->db->get_where('t_menu', array('id'=>$val['parent_id']))->row_array();
					if (!empty($parent)) {
						$parent = $parent['title'];
					} else {
						$parent = "-";
		...
		if ( $this->write_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				$this->form_validation->set_rules(array(
					...
					array(
						'field' => 'url',
						'label' => lang_line('form_label_url'),
				...
				if ( $this->form_validation->run() ) 
				{
					...
					$posisi = $this->db->get_where('t_menu', array('group_id'=>'4'))->row_array();
					$posisi = $posisi['position']+1;

					$active = ($this->input->post('active') == '1' ? 'Y' : 'N');
					$data_form = array(
						'title'    => xss_filter($this->input->post('menu')),
						// 'parent_id' => xss_filter($this->input->post('parent_id')),
			...
			$this->vars['parent'] = $this->db->get_where('t_menu', array('group_id'=>'4'))->result();
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_menuwebsite($id_page);
					$this->vars['parent'] = $this->db->get_where('t_menu', array('group_id'=>'4'))->result();
