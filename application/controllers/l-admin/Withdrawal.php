...
class Withdrawal extends Admin_controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('mod/withdrawal_model', 'mod_model');
        ...
        if ($this->modify_access) {
            if (!empty($id_page) && $this->mod_model->cek_id($id_page) == 1) {
                if ($this->input->is_ajax_request()) {
                    ...
                    $this->vars['res_pages'] = $this->mod_model->get_withdraw($id_page);
                    $this->vars['bank'] = $this->bank_model->get_bank_user($this->vars['res_pages']['bank_id']);
                    $this->vars['wallet'] = $this->wallet_model->get_wallet_user($this->vars['res_pages']['user_id']);
        ...
        if ($this->input->is_ajax_request() && !empty($param)) {
            ...
            if ($this->modify_access && $this->mod_model->cek_id($pk) == 1) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    $wdlalu = $this->mod_model->get_withdraw($pk);
                    $user = $this->member_model->get_user($wdlalu['user_id']);
                    //periksa status sebelumnya
                    //jika status sebelumnya adalah PENDING
                    if ($wdlalu['status'] == 'Pending') {
                        ...
                        //jika status yang dipilih admin bukan PENDING
                            ...
                            if ($status == 'Rejected') {
                                ...
                                $wallet = $this->wallet_model->get_wallet_user($wdlalu['user_id']);

                                $wallet_sebelumnya = $wallet['amount'];
                                $jml_wd = $wdlalu['amount_usd'];

                                //menambahkan wallet dengan jumlah wd
                                $jumlah_wallet = $wallet_sebelumnya + $jml_wd;
                                ...
                                Mohon maaf, pengajuan withdrawal anda telah kami <strong>tolak</strong>. Silakan mengajukan withdrawal kembali.<br><p><p>
								Best regards, <p><p><p>
                            ...
                            elseif ($status == 'Approved') {
                                ...
                                $message = '<html><body>
                                Dear <b> ' . $user["user_name"] . ' </b>,<p /><p />
                                Selamat, pengajuan withdrawal anda telah kami <strong>terima</strong>.<br><p><p>
                                Best regards, <p><p><p>
    ...
    public function sendEmail($user, $msg)
    {
        ...
        // Subject email
        $this->email->subject('Your Withdrawal Status ');
