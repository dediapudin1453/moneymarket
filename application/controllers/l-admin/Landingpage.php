...
class Landingpage extends Admin_controller {
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/landingpage_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$row[] = '<div>'.cut($val['content'],60,TRUE).'...</div>';
		...
		if ( $this->write_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				if ( $this->form_validation->run() ) 
				{
					...
					$data_form = array(
						'name'    => xss_filter($this->input->post('title')),
						// 'seotitle' => seotitle($this->input->post('title')),
						'comment'  => xss_filter($this->input->post('content')),
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_homepage($id_page);
	...
	private function _submit_upate($param = NULL)
	{
		if ( $this->input->is_ajax_request() && !empty($param) ) 
		{
			...
			if ( $this->modify_access && $this->mod_model->cek_id($pk) == 1 ) 
			{
				...
				{
					$data = array(
						'section'    => xss_filter($this->input->post('title')),
						// 'seotitle' => seotitle($this->input->post('title')),
