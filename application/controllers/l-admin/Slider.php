...
class Slider extends Admin_controller {
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/slider_model','mod_model');
	}
			...
			if ( $this->input->is_ajax_request() ) 
			{
				$this->form_validation->set_rules(array(
					array(
						...
						'rules' => 'trim|min_length[3]|max_length[100]|callback__cek_add_seotitle'
					)
				));
			...
			if ( $this->modify_access && $this->mod_model->cek_id($pk) == 1 ) 
			{
				$rules = array(
					array(
						'field' => 'title',
						'label' => lang_line('form_label_title'),
						'rules' => 'trim|min_length[3]|max_length[100]|callback__cek_edit_seotitle'
					)
				);
