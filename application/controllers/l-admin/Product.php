...
class Product extends Admin_controller {
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/product_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$headline = ( $val['highlight'] == 'Y' ? '<span class="h-'. $val['id'] .'"><i class="fa fa-star text-warning"></i></span>' : '<span class="h-'. $val['id'] .'"></span>');

					$row[] = '<div>'. $val['title'] . ' '. $headline;
					$row[] = htmlspecialchars_decode($val['content']);
					$row[] = '<img src="'.site_url().'content/uploads/'.$val['picture'].'" style="width:100%;max-width:35px;">';
					// $val['picture']; 
