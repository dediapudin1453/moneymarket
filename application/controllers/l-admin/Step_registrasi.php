...
class Step_registrasi extends Admin_controller {

	public $mod = 'step_registrasi';
	public $pk;
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/step_registrasi_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$row[] = $val['title'];
					$row[] = $val['subtitle'];
					$row[] = htmlspecialchars_decode($val['information']);
					// $val['picture']; 
			...
			if ( $this->input->is_ajax_request() ) 
			{
				...
				if ( $this->form_validation->run() ) 
				{
					$data_form = array(
						...
						'subtitle' => xss_filter($this->input->post('subtitle')),
						'information'  => xss_filter($this->input->post('content'))
					);
	...
	public function submit_add()
	{
		if ( $this->write_access )
		{
			...
			else 
			{
				...
				redirect(admin_url($this->mod.'/edit/'.$pk));
			}
		}
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_step_registrasi($id_page);
