...
class Chat_whatsapp extends Admin_controller {

	public $mod = 'chat_whatsapp';
	public $pk;
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/chat_whatsapp_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$row[] = substr(htmlspecialchars_decode($val['chat']), 0, 50);
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_slider($id_page);
	...
	private function _submit_upate($param = NULL)
	{
		if ( $this->input->is_ajax_request() && !empty($param) ) 
		{
			...
			{
				$data = array(
					'chat'    => xss_filter($this->input->post('chat'))
				);
