	...
	*/
	...
	public function submit_withdrawal()
	{
		...
		if ( $this->form_validation->run()) {
			...
			if (!empty($cek_bank)) {
	        	...
	        	if ($amount > $wallet['amount']) {
	        		$this->session->set_flashdata('pesan', '<div class="alert alert-danger">
				  		<strong>Danger!</strong> '.lang_line('withdrawal_label_walleterror').'.
					</div>');
	        	...
	        	elseif ($amount <= 0) {
	        		$this->session->set_flashdata('pesan', '<div class="alert alert-danger">
				  		<strong>Danger!</strong> '.lang_line('withdrawal_label_amounterror').'.
					</div>');
					redirect(member_url('withdrawal'));
	        	} 
				...
				$this->session->set_flashdata('pesan', '<div class="alert alert-danger">
				  <strong>Danger!</strong> '.lang_line('withdrawal_label_emptybank').'.
				</div>');
				redirect(member_url('withdrawal'));
			}
		...
		else
		{
			...
			$this->session->set_flashdata('pesan', '<div class="alert alert-danger">
			  <strong>Danger!</strong> '.$error_content.'.
			</div>');
			redirect(member_url('withdrawal'));
		}
	}
