...
$('#form_update_identity').on('submit',function(e){
    e.preventDefault();
    $('.submit_update_identity').find('i').removeClass().addClass('icon-spinner2 spinner mr-2');
    $('.noty_layout').remove();
    var formData = new FormData(this);
    var form = $('#form_update_identity');
    $.ajax({
        url: admin_url + a_mod + '/submit-update-user',
        type: 'POST',
        //data: formData.serialize(),
...
$(".close-preview3").click(function(){
    $(this).hide();
    var resimg = $('#resset-image').val();
    var valimg = $(this).attr('vaimg');
    $('#image-preview2').attr('src', resimg);
    $('#upload-image2').val(valimg);
    
});

$(".close-preview4").click(function(){
    $(this).hide();
    var resimg = content_url + '/images/avatar.jpg';
    $('#image-preview2').attr('src', resimg);
    $('#upload-image2').val('');
