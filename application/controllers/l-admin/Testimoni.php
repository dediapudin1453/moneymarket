...
class Testimoni extends Admin_controller {
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/testimoni_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$row[] = '<div>'.cut($val['comment'],60,TRUE).'...</div>';
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_testimoni($id_page);
