...
class Footer extends Admin_controller {
	...
	public function __construct() 
	{
		...
		$this->load->model('mod/footer_model','mod_model');
	}
		...
		if ( $this->read_access )
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				foreach ($data_list as $val) 
				{
					...
					$row[] = '<span class="badge badge-b badge-pill badge-primary">Active</span>';
	...
	public function edit($id_page = '')
	{
		...
		if ( $this->modify_access )
		{
			...
			{			
				...
				{
					$this->vars['res_pages'] = $this->mod_model->get_footer($id_page);
		...
		if ( $this->input->is_ajax_request() && !empty($param) ) 
		{
			...
			if ( $this->modify_access && $this->mod_model->cek_id($pk) == 1 ) 
			{
				$rules = array(
					array(
						'field' => 'copyright',
						'label' => lang_line('form_label_copyright'),
				...
				if ( $this->form_validation->run() ) 
				{
					// $active = ($this->input->post('active') == '1' ? 'Y' : 'N');
					$data = array(
						'copyright'    => xss_filter($this->input->post('copyright')),
					);
